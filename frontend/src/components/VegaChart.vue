...
const selectedMetrics = ref(['deep_sleep_duration', 'rem_sleep_duration', 'total_sleep_duration'])
...
function getEmbedOptions() {
  return {
    ...baseOpt,
    downloadFileName: fileName.value,
  }
}
function embedChart() {
  if (vis.value) {
    const plainSpec = JSON.parse(JSON.stringify(vegaSpec.value))
    embed(vis.value, plainSpec, getEmbedOptions())
      .then(({ view }) => {
...
watch([selectedMetrics, xAxisLabel, yAxisLabel, fileName], () => {
  ...
  embed(vis.value, newSpec, getEmbedOptions())
    .then(({ view }) => {
      chartView.value = view
